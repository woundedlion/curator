...
apiClient.submitSpotifyRequest()
     ├── RequestQueue.acquire()      (token bucket: paces outbound rate)
     └── CircuitBreaker.tryAcquire() (handles 429 recovery via half-open probe)
```

Both submodules live in their own files (`requestQueue.ts`, `circuitBreaker.ts`) and own clean state machines. `apiClient.ts` is the thin composition layer that wires them together and translates HTTP status codes to typed errors.

1. **One submission API.** `submitSpotifyRequest<T>(request, clientId)` is the single chokepoint for Bearer-authenticated calls; `submitTokenRequest(send, path)` is the equivalent for `accounts.spotify.com/api/token` requests (different host, no Bearer, but the same per-app quota — so it shares the queue and breaker). Both go through the same internal `submitRaw` that enforces queueing → breaker check → send → status mapping. **Impossible to bypass** — no other module is permitted to `fetch()` Spotify directly.
2. **Token-bucket request queue.** Capacity 1; one token regenerates every `MIN_REQUEST_SPACING_MS = 350`. Sustained throughput ceiling ~2.85 req/sec, leaving headroom for SDK / token refresh / sidebar refresh noise that share the same quota. (The previous 250ms target sat right at the edge of what tighter accounts allow and consistently 429'd on re-enrich-all of large playlists.) **Pre-allocate, then await:** slots are reserved synchronously at `acquire()` entry (`mySlotAt = max(now, nextTokenAt); nextTokenAt = mySlotAt + spacing`), then the function awaits any required wait. This eliminates the same-frame burst that the old "wait, then claim" pattern produced when limiter-released callers all read the same `nextTokenAt` and raced on wake-up. **Pending count** is exposed via `getPendingSpotifyRequestCount()` — the Toolbar surfaces it as `Spotify · N queued` next to the MB enrichment counter.
3. **Persistence across reload.** The queue's `nextTokenAt` and the breaker's `openUntil` are both persisted to localStorage. Spotify's rolling-30s quota counts requests from the previous page session; without persistence a refresh that immediately follows a heavy burst would fire its first request unspaced and walk straight into the unfinished window. Both timestamps are capped on read (60s for the queue, 1h for the breaker) so a corrupt or stale value can't deadlock the limiter forever.
...
6. **`Retry-After` parsing with a pessimistic fallback.** Reads delta-seconds or HTTP-date per RFC 9110 §10.2.3. **When the header is unparseable or missing, defaults to 5 minutes**, not 30 seconds. The 30s default was responsible for the 23-hour incident: Spotify includes `Retry-After` on the wire even on multi-hour bans, but the header is **not CORS-safelisted**, so the JS `Headers.get('Retry-After')` returns `null` unless Spotify includes it in `Access-Control-Expose-Headers` (and for sustained-abuse paths they don't). The breaker would re-fire after 30 seconds straight into a still-active 23-hour ban. 5 minutes is short enough not to strand on a benign 429, long enough to break the retry-into-ban loop on a hidden multi-hour penalty. The 429 log records `corsHidden: true` when this fallback triggers so the cause is diagnosable from the console.
...
The whole machine is covered by `src/spotify/rateLimit.test.ts` (mock-fetch + fake timers): fresh load, global refresh, manual item refresh, half-open probe success / failure, breaker persistence across module reload, "N concurrent callers, one 429, exactly one outbound request fires" invariant. The deletion-cancellation contract is covered by `src/services/deletionCancellation.test.ts`: cancelByTag direct queue behaviour, store-action wiring (removeTrack / removeTracks / clearPlaylist / replaceAll), defence-in-depth guard at the task-run boundary, repeated delete cycles (re-add → re-cancel under the same id), interleaved deletes during heavy traffic, reorder/augment between deletions doesn't disturb anything.
