...
On drop, the moving ids are placed via `moveSelectionMaintainingShape(visibleIds, selection, activeId, overId)`. The semantics: the **internal shape of the selection is preserved** — every selected row keeps its original offset from the grabbed (active) row, so the gaps between selected rows stay constant and unselected rows flow one at a time *through* those gaps as the selection moves. Gaps collapse **only** when the selection is pushed against an array boundary (a selected row would otherwise land off the end). The active row's landing slot is derived by counting only the UNSELECTED rows the cursor has crossed, so dragging past one row shifts the selection by exactly one slot — even when selected rows sit between the active row and the cursor. (Deriving it from the raw over-index instead makes a 2-row selection lurch 2 slots at a time, which reads as "it won't move one-by-one.") Worked examples for `[A, B, C, D]`, selection `{A, C}`:

- Grab A, drag down over B → A→1, C keeps its +2 offset to 3, `[B, D]` fill `[0, 2]` → **`[B, A, D, C]`**.
- Grab A, drag down over D → C would want index 4 (off the end), so the gap collapses at the boundary → **`[B, D, A, C]`**.

So the selection only collapses to contiguous when a gap is forced off an end. If the drop target is itself part of the selection, no move occurs (the "drop on self" guard, also used for stability — see the live preview note below). One reorder undo entry is pushed for the whole move.

**Live preview during the drag** — the reflow the user will get on release is rendered *while* they're dragging, not just after mouseup. On each `dnd-kit` `onDragOver` event the table recomputes the visible-id order with the same `moveSelectionMaintainingShape` call the drop path uses (so what you see is what you get) and feeds that order into both the `SortableContext.items` and the virtualizer's per-row track lookup. The effect: unselected rows slide out of the way and the non-active selected rows visibly slot into their landing positions — the post-drop layout assembles under the cursor. While a multi-row drag is in flight the per-row dnd-kit sorting transform is suppressed (`multiDragActive`) so the preview order is the sole positioner — otherwise the virtualizer's reorder and dnd-kit's strategy transform would double up and visually tear the block apart. The grabbed row follows the pointer via the `DragOverlay`; the selected rows fade to 60% opacity (via the `partOfActiveMultiDrag` prop) so the whole moving block reads as one group.
